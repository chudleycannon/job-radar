...
actually exist in the bundled list, out of 17,811 sources:
...
| `countries` | list of codes | Only drops a source whose country tag names somewhere else, which is 11,000 of the 17,811. The other 6,811 are fetched whatever you set: 5,214 carry no tag, and 1,597 are tagged `multi`, meaning the board belongs to a multinational rather than to one country. Neither is evidence the employer has nothing where you are, and a multinational is one of the likelier places to find a vacancy in your country, so both are kept. `[UK]` therefore leaves 7,745 sources rather than the 934 tagged `UK`. This trims what gets fetched; `locations.countries` is what decides where a role is. |

...
**17,811 employer boards across 24 platforms**, plus six cross-employer
feeds: LinkedIn three times over, once unscoped and once each for postings
LinkedIn itself marks contract and temporary, plus NHS Jobs and Workable's own
search, which are keyword templates expanded against your titles, and
Workable's recently-posted feed, which takes no keyword. That is what takes `sources/sources.json` to 17,817
...
prints rather more than 17,817, because each of the five keyword templates
...
Of the 17,817 sources, 5,215 carry no country tag at all and 1,597 are tagged
...
find a vacancy in your country. So `sources.countries: [UK]` leaves 7,750
sources, not the 938 that carry a `UK` tag. It is a way to skip the boards you
...
- **The right to work.** A posting that states its sponsorship position is
  ...
  available.
- **Sector filtering is only as good as the tags.** Of 17,817 sources, 6,093
  carry a sector tag and 11,724 carry none, because an address harvested from
